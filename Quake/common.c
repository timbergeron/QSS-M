...
static void COM_CheckRegistered (void)
{
	...
	if (i != (int) sizeof(check) ||
		!QuakeDataImport_ValidatePopData(check, sizeof(check)))
...
void COM_InitFilesystem (void) //johnfitz -- modified based on topaz's tutorial
{
	...
	import_userdir = host_parms->userdir != host_parms->basedir ?
		host_parms->userdir : com_basedir;
	/* Import is advisory: PK3-only and loose-file installations are valid, and
	 * cancelling must leave final validation to the normal filesystem startup. */
	(void)QuakeDataImport_RunAtStartup(com_basedir, import_userdir, isDedicated,
		com_missingpak_hint, sizeof(com_missingpak_hint));
