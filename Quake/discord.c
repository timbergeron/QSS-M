	...
	discord_presence_kind_t kind, const char *map)
{
	...
	if (!Mapshot_ResolveURL(map, gamedir, url, sizeof(url)))
		return;
...
static const char *Discord_ModeLabel(const char *mode)
{
...
static const char *Discord_PlaymodeLabel(const char *playmode)
{
	...
	const char *map, const char *mod_label)
{
	...
	mode_label = Discord_ModeLabel(mode[0] ? mode : gametype);
	playmode_label = Discord_PlaymodeLabel(playmode);
...
static void DiscordPresence_Init(void)
{
...
static void DiscordPresence_Shutdown(void)
{
...
static void DiscordPresence_Frame(void)
{
	double now;
	qboolean hidden;
...
static int DiscordNotifyThread(void *data);
static qboolean DiscordAlert_BuildPayload(const discord_alert_t *alert,
	char *out, size_t outsz);
static qboolean DiscordNotify_Enqueue(const char *raw_msg,
	const char *chat_body, discord_alert_kind_t kind, qboolean is_test);
...
static void DiscordNotify_Callback(cvar_t* var)
{
...
static int DiscordNotifyThread(void *data)
{
	...
	discord_test_status_t test_status = DISCORD_TEST_TRANSPORT_ERROR;
	long http_code = 0;
    ...
    if (curl) {
		...
		if (hdr)
		{
			...
			for (attempt = 0; attempt < 2; ++attempt)
			{
				...
				if (rc != CURLE_OK)
				{
					test_status = DISCORD_TEST_TRANSPORT_ERROR;
					break;
				}
				...
				{
					test_status = DISCORD_TEST_DELIVERED;
					break;
				}
				...
				test_status = DISCORD_TEST_HTTP_ERROR;
				break;
			}
		}
	...
	else if (test_status != DISCORD_TEST_DELIVERED)
...
static void DiscordAlert_CaptureMode(discord_alert_t *alert)
{
	...
	mode_label = Discord_ModeLabel(raw_mode);
	playmode_label = Discord_PlaymodeLabel(playmode);
...
static void DiscordAlert_CaptureContext(discord_alert_t *alert)
{
	...
	Info_GetKey(cl.serverinfo, "hostname", server_name, sizeof(server_name));
	DiscordAlert_CleanText(alert->server, sizeof(alert->server), server_name);
...
static qboolean DiscordAlert_BuildSnapshot(discord_alert_t *alert,
	const char *raw, const char *chat_body, discord_alert_kind_t kind,
	qboolean is_test)
{
	...
	if (kind == DISCORD_ALERT_MENTION)
		parsed = DiscordAlert_ParsePlayerChat(alert, raw, chat_body);
	...
	const discord_alert_t *alert)
{
	...
	if (!thread)
	{
		...
		if (!alert->is_test)
			DiscordNotify_SetFailure(DISCORD_TEST_TRANSPORT_ERROR, 0);
		free(job);
		DiscordNotifyWorker_End();
		return DISCORD_ALERT_SEND_DROPPED;
	}
...
static qboolean DiscordAlert_Enqueue(const discord_alert_t *alert)
{
	if (discord_alert_queue_count >= DISCORD_ALERT_QUEUE_MAX)
	{
		...
		if (discord_alert_queue[0].is_test)
			DiscordTest_SetResult(DISCORD_TEST_TRANSPORT_ERROR, 0);
	...
	const char *chat_body, discord_alert_kind_t kind, qboolean is_test)
{
	...
	DiscordAlert_BuildSnapshot(&alert, raw_msg, chat_body, kind, is_test);
...
{
	DiscordNotify_Enqueue(raw_message, chat_body, DISCORD_ALERT_MENTION, false);
}

void Discord_NotifyDirectMessage(const char *raw_message)
{
	DiscordNotify_Enqueue(raw_message, NULL, DISCORD_ALERT_DIRECT_MESSAGE, false);
}
...
discord_test_status_t Discord_NotifyTest(void)
{
	...
	if (!con_notifydiscord.string[0] || !IsDiscordWebhookURL(con_notifydiscord.string))
		return DISCORD_TEST_NOT_CONFIGURED;

	DiscordTest_SetResult(DISCORD_TEST_SENDING, 0);
	if (!DiscordNotify_Enqueue("Menu Discord alert test", NULL,
		DISCORD_ALERT_TEST, true))
	{
		DiscordTest_SetResult(DISCORD_TEST_TRANSPORT_ERROR, 0);
		return DISCORD_TEST_TRANSPORT_ERROR;
	}
	return DISCORD_TEST_SENDING;
}
...
static int DiscordCommunityThread(void *unused)
{
	...
	if (DiscordCommunity_FetchInviteCode(invite_code, sizeof(invite_code)) &&
		...
		DiscordCommunity_SetResult(DISCORD_COMMUNITY_READY, online_count);
	else
		DiscordCommunity_SetResult(DISCORD_COMMUNITY_ERROR, 0);
	DiscordWorker_End();
	return 0;
}
...
void Discord_CommunityRefresh(void)
{
	...
	if (status == DISCORD_COMMUNITY_LOADING)
		return;
	if ((status == DISCORD_COMMUNITY_READY ||
		status == DISCORD_COMMUNITY_ERROR) && now - completed < cache_ms)
		return;
...
void Discord_Init(void)
{
	...
	discord_notify_next_warning = 0.0;
	DiscordAlert_ResetFollowup();
	...
	Cvar_RegisterVariable(&con_notifydiscord);
	Cvar_SetCallback(&con_notifydiscord, &DiscordNotify_Callback);
