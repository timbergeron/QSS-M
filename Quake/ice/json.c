...
#ifdef HAVE_JSON
#define MAX_JPATH 128	//arbitrary limit for buffers. must be large enough to contain a keyname or a float.
	...
	if (parent)
	{
		if (parent->arraymax)
		{
			...
			if (idx >= parent->arraymax)
			{
				...
				parent->arraymax*=2;	//double it...
				if (parent->arraymax < idx+1)
					parent->arraymax = idx+1;	//extend it more if there's a gap somehow.
...
uintptr_t JSON_GetUInteger(json_t *t, const char *child, unsigned int fallback)
{
	...
	{	//copy it to another buffer. can probably skip that tbh.
		...
		if (!strcmp(tmp, "true"))	//special cases, for booleans
			return 1u;
		r = (uintptr_t)strtoull(tmp, &trail, 0);
		if (!*trail)
			return r;
	}
	return fallback;
...
intptr_t JSON_GetInteger(json_t *t, const char *child, int fallback)
{
	...
	{	//copy it to another buffer. can probably skip that tbh.
		...
		if (!strcmp(tmp, "true"))	//special cases, for booleans
			return 1;
		r = (intptr_t)strtoll(tmp, &trail, 0);
		if (!*trail)
			return r;
	}
	return fallback;
