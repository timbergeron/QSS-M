...
#define GNUTLS_FUNCS \
	...
	#define GNUTLS_FUNC(n,ret,args) static ret (*q##n)args;
...
static void SSL_SetCertificateName(gnutlsstream_t *f, const char *hostname)
{
	int i;
	if (hostname)
	{
...
static void SSL_Close(icestream_t *vfs)
{
	gnutlsstream_t *file = (void*)vfs;
...
#ifdef HAVE_TLS
static qboolean SSL_CloseFile(icestream_t *vfs)
{
	gnutlsstream_t *file = (void*)vfs;
	SSL_Close(vfs);
	if (file->stream)
	{
		file->stream->Close(file->stream);
		file->stream = NULL;
	}
	if (file->certcred)
		qgnutls_certificate_free_credentials(file->certcred);
...
static int SSL_CheckUserTrust(gnutls_session_t session, gnutlsstream_t *file, int gcertcode)
{
...
static int SSL_CheckCert(gnutls_session_t session)
{
	gnutlsstream_t *file = qgnutls_session_get_ptr (session);
...
#ifdef HAVE_DTLS
static int SSL_CheckFingerprint(gnutls_session_t session)
...
//-1 for error
//0 for not ready
static int SSL_DoHandshake(gnutlsstream_t *file)
{
	int err;
...
#ifdef HAVE_TLS
static int SSL_Read(struct icestream_s *f, void *buffer, int bytestoread)
{
	gnutlsstream_t *file = (void*)f;
	int read;
	...
	if (read < 0)
	{
		...
		{
			Con_DPrintf("TLS Premature Termination from %s\n", file->certname);
...
static int SSL_Write(struct icestream_s *f, const void *buffer, int bytestowrite)
{
	gnutlsstream_t *file = (void*)f;
	int written;
...
{
	gnutlsstream_t *file = p;
//	Sys_Printf("SSL_Push: %u\n", size);
	int done = file->stream->WriteBytes(file->stream, data, size);
	if (done <= 0)
	{
		int eno;
		file->pusherror = done;
...
//	Sys_Printf("SSL_Pull: %u\n", size);
	int done = file->stream->ReadBytes(file->stream, data, size);
	if (done <= 0)
	{
		int eno;
		file->pullerror = done;
...
//static qboolean	servercertfail;	//set when we failed to load a cert to use(as server). psk or per-session certs are valid alternatives.
#endif
#ifdef HAVE_DTLS
	...
	if (i++)
	{
		if (displayname)
			if (!FS_DisplayPath(com_argv[i], FS_SYSTEM, displayname, displaysize))
				q_strlcpy(displayname, com_argv[i], displaysize);
...
static qboolean SSL_InitGlobal(qboolean isserver)
{
	int err;
	isserver = !!isserver;
...
#define ICE_UNUSED_FN __attribute__((unused))
#else
#define ICE_UNUSED_FN
#endif
static void ICE_UNUSED_FN GnuTLS_Shutdown(void)
{
	int isserver;
...
#endif
static qboolean SSL_InitConnection(gnutlsstream_t *newf, qboolean isserver, qboolean datagram)
{
	...
	if (datagram)
	{
		...
		qgnutls_transport_set_push_function(newf->session, DTLS_Push);
		qgnutls_transport_set_pull_function(newf->session, DTLS_Pull);
#endif
	}
	else
	{
...
#ifdef HAVE_TLS
static icestream_t *GNUTLS_OpenVFS(const char *hostname, icestream_t *source, qboolean isserver)
{
	gnutlsstream_t *newf;

	if (source && SSL_InitGlobal(isserver))
	{
		newf = calloc(1, sizeof(*newf));
		if (newf)
		{
			newf->stream = source;
...
#if 0
static int GNUTLS_GetChannelBinding(icestream_t *vf, qbyte *binddata, size_t *bindsize)
{
	gnutls_datum_t cb;
	gnutlsstream_t *f = (gnutlsstream_t*)vf;
...
{
	int ret;
	gnutlsstream_t *f = (gnutlsstream_t *)ctx;
...
static neterr_t GNUDTLS_Received(void *ctx, netadr_t *from, const void *src_data, size_t src_size, void *out_data, size_t out_maxsize, size_t *out_used)
{
	int ret;
	gnutlsstream_t *f = (gnutlsstream_t *)ctx;
	...
	if (f->challenging)
	{
		...
		{
		case AF_INET:	asize = sizeof(from->in);	break;
		case AF_INET6:	asize = sizeof(from->in6);	break;
		default: return NETERR_NOROUTE;
		}
...
static size_t Base16_EncodeBlock(const qbyte *in, size_t length, char *out, size_t outsize)
{
...
static qboolean GNUDTLS_GenTempCertificate(const char *subject, struct dtlslocalcred_s *qcred)
{
	...
	//we should probably just go and make our own root ca/master. post it a cert and get a signed one (with sequential serial) back or something.
	//we'll probably want something like that for client certs anyway, for stat tracking.
	q_snprintf(serial, sizeof(serial), "%u", (unsigned)time(NULL));
...
//	qgnutls_x509_crt_set_key_usage(cert, GNUTLS_KEY_KEY_ENCIPHERMENT|GNUTLS_KEY_DATA_ENCIPHERMENT|);
	...
	memcpy(qcred->cert = malloc(pub.size), pub.data, pub.size);

	qcred->keysize = priv.size;
	memcpy(qcred->key = malloc(priv.size), priv.data, priv.size);
...
#else
#error "GNUTLS version is too old (3.0+ required)."
#endif
...
{
	return GNUTLS_OpenVFS(hostname, source, isserver);
}
#endif
