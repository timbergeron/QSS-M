...
static int PF_ChainFieldOffset(void)
{	//menuqc declares no system fields, so chain cannot be assumed to sit at the entvars offset,
	//and a same-named field of some other type would link garbage rather than edict references.
	ddef_t *def = ED_FindField("chain");
...
//Quake rerelease debug drawing; accepted for compatibility but not rendered.
	{"ex_draw_point",	PF_void_stub,		PF_NoCSQC,			0/*81*/,PF_NoMenu,	"QSSDEP void(vector point, float colormap, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_line",	PF_void_stub,		PF_NoCSQC,			0/*82*/,PF_NoMenu,	"QSSDEP void(vector start, vector end, float colormap, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_arrow",	PF_void_stub,		PF_NoCSQC,			0/*83*/,PF_NoMenu,	"QSSDEP void(vector start, vector end, float colormap, float size, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_ray",		PF_void_stub,		PF_NoCSQC,			0/*84*/,PF_NoMenu,	"QSSDEP void(vector start, vector direction, float length, float colormap, float size, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_circle",	PF_void_stub,		PF_NoCSQC,			0/*85*/,PF_NoMenu,	"QSSDEP void(vector origin, float radius, float colormap, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_bounds",	PF_void_stub,		PF_NoCSQC,			0/*86*/,PF_NoMenu,	"QSSDEP void(vector min, vector max, float colormap, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_worldtext",PF_void_stub,		PF_NoCSQC,			0/*87*/,PF_NoMenu,	"QSSDEP void(string s, vector origin, float size, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_sphere",	PF_void_stub,		PF_NoCSQC,			0/*88*/,PF_NoMenu,	"QSSDEP void(vector origin, float radius, float colormap, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
	{"ex_draw_cylinder",PF_void_stub,		PF_NoCSQC,			0/*89*/,PF_NoMenu,	"QSSDEP void(vector origin, float halfHeight, float radius, float colormap, float lifetime, float depthtest)", "stub. Quake rerelease debug drawing is accepted and ignored."},
...
//	{"pushmove", 		PF_pushmove, 		PF_pushmove,	 	0, 		PF_NoMenu, "float(entity pusher, vector move, vector amove)"},
	{"frameforname",	PF_frameforname,	PF_frameforname,	276,	PF_frameforname,276, D("float(float modidx, string framename)", "Looks up a framegroup from a model by name, avoiding the need for hardcoding. Returns -1 on error.")},// (FTE_CSQC_SKELETONOBJECTS)
	{"frameduration",	PF_frameduration,	PF_frameduration,	277,	PF_frameduration,277, D("float(float modidx, float framenum)", "Retrieves the duration (in seconds) of the specified framegroup.")},// (FTE_CSQC_SKELETONOBJECTS)
...
//	{"skel_set_bone_world",PF_skel_set_bone_world,PF_skel_set_bone_world,283,PF_NoMenu, D("void(entity ent, float bonenum, vector org, optional vector angorfwd, optional vector right, optional vector up)", "Sets the world position of a bone within the given entity's attached skeletal object. The world position is dependant upon the owning entity's position. If no orientation argument is specified, v_forward+v_right+v_up are used for the orientation instead. If 1 is specified, it is understood as angles. If 3 are specified, they are the forawrd/right/up vectors to use.")},
	{"frametoname",		PF_frametoname,		PF_frametoname,		284,	PF_frametoname,0, "string(float modidx, float framenum)"},
...
//	{"entityprotection",NULL,				NULL,				0,		PF_NoMenu, D("float(entity e, float nowreadonly)", "Changes the protection on the specified entity to protect it from further edits from QC. The return value is the previous setting. Note that this can be used to unprotect the world, but doing so long term is not advised as you will no longer be able to detect invalid entity references. Also, world is not networked, so results might not be seen by clients (or in other words, world.avelocity_y=64 is a bad idea).")},
	...
	{"findflags",		PF_findflags,		PF_findflags,		449,	PF_findflags,87, "entity(entity start, .float fld, float match)"},//DP_QC_FINDFLAGS
...
//	{"loadfromfile",	PF_loadfromfile,	PF_loadfromfile,	530,	PF_NoMenu, D("void(string s)", "Reads a set of entities from the named file. This file should have the same format as a .ent file or a saved game. Entities will be spawned as required. If you need to see the entities that were created, you should use parseentitydata instead.")},
	...
	{"getresolution",			PF_NoSSQC,	PF_cl_getresolution,		608,PF_cl_getresolution,		608, D("vector(float mode, optional float forfullscreen)", "Returns available/common video modes.")},
...
//		fprintf(f, "const float STAT_PUNCHVECTOR_Z = 31;\n");
	...
	fprintf(f, "const float STAT_QSSM_WHEEL_WEAPONS = %i;	/* QSS-M opt-in EV_INTEGER clientstat for canonical weapon wheel ownership bits. */\n", STAT_QSSM_WHEEL_WEAPONS);
	fprintf(f, "const float STAT_QSSM_WHEEL_ACTIVE = %i;	/* QSS-M opt-in EV_INTEGER clientstat for the current weapon in canonical wheel bits. */\n", STAT_QSSM_WHEEL_ACTIVE);
