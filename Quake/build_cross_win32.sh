...
PREFIX="${CROSS_PREFIX:-${PREFIX:-/opt/cross_win32}}"
...
exec "$MAKE_CMD" WINSOCK2=1 CC="$CC" AS="$AS" RANLIB="$RANLIB" AR="$AR" WINDRES="$WINDRES" STRIP="$STRIP" PKG_CONFIG="$PKG_CONFIG" -f Makefile.w32 "$@"
